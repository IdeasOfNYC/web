  ...
  useEffect(() => {
    ...
    if (ideaFilter.borough) {
      ...
      if (selectedBoroughPath) {
        ...
        svgElement.style.transform = `translate(${translateX}%, ${translateY}%) scale(1.5)`;
    ...
    >
      <div className="z-10">
        <div className="absolute bottom-8 right-8 flex flex-col gap-2">
          {ideaFilter.borough !== null ? (
            <button
              className="cursor-pointer bg-white px-8 py-4 rounded-md border border-neutral-400 border-dashed hover:bg-neutral-50"
              onClick={() => setIdeaFilter({ ...ideaFilter, borough: null })}
            >
              Back
            </button>
          ) : (
      ...
      >
        ...
        <path
          ...
          className={`${
            ideaFilter.borough === null
              ? "fill-white stroke-neutral-400 hover:fill-green-50 hover:stroke-green-500"
              : ideaFilter.borough === "staten island"
              ? "fill-white stroke-neutral-400 hover:fill-neutral-50"
              : "fill-white stroke-neutral-200 hover:fill-green-50 hover:stroke-green-500"
          }`}
          onClick={() => {
            if (ideaFilter.borough === "staten island")
              setIdeaFilter({ ...ideaFilter, borough: null });
            else setIdeaFilter({ ...ideaFilter, borough: "staten island" });
          }}
        />
        {/* brooklyn */}
        <path
          ...
          className={`${
            ideaFilter.borough === null
              ? "fill-white stroke-neutral-400 hover:fill-green-50 hover:stroke-green-500"
              : ideaFilter.borough === "brooklyn"
              ? "fill-white stroke-neutral-400 hover:fill-neutral-50"
              : "fill-white stroke-neutral-200 hover:fill-green-50 hover:stroke-green-500"
          }`}
          onClick={() => {
            if (ideaFilter.borough === "brooklyn")
              setIdeaFilter({ ...ideaFilter, borough: null });
            else setIdeaFilter({ ...ideaFilter, borough: "brooklyn" });
          }}
        />
        {/* queens */}
        <path
          ...
          className={`${
            ideaFilter.borough === null
              ? "fill-white stroke-neutral-400 hover:fill-green-50 hover:stroke-green-500"
              : ideaFilter.borough === "queens"
              ? "fill-white stroke-neutral-400 hover:fill-neutral-50"
              : "fill-white stroke-neutral-200 hover:fill-green-50 hover:stroke-green-500"
          }`}
          onClick={() => {
            if (ideaFilter.borough === "queens")
              setIdeaFilter({ ...ideaFilter, borough: null });
            else setIdeaFilter({ ...ideaFilter, borough: "queens" });
          }}
        />
        {/* manhattan */}
        <path
          ...
          className={`${
            ideaFilter.borough === null
              ? "fill-white stroke-neutral-400 hover:fill-green-50 hover:stroke-green-500"
              : ideaFilter.borough === "manhattan"
              ? "fill-white stroke-neutral-400 hover:fill-neutral-50"
              : "fill-white stroke-neutral-200 hover:fill-green-50 hover:stroke-green-500"
          }`}
          onClick={() => {
            if (ideaFilter.borough === "manhattan")
              setIdeaFilter({ ...ideaFilter, borough: null });
            else setIdeaFilter({ ...ideaFilter, borough: "manhattan" });
          }}
        />
        {/* bronx */}
        <path
          ...
          className={`${
            ideaFilter.borough === null
              ? "fill-white stroke-neutral-400 hover:fill-green-50 hover:stroke-green-500"
              : ideaFilter.borough === "bronx"
              ? "fill-white stroke-neutral-400 hover:fill-neutral-50"
              : "fill-white stroke-neutral-200 hover:fill-green-50 hover:stroke-green-500"
          }`}
          onClick={() => {
            if (ideaFilter.borough === "bronx")
              setIdeaFilter({ ...ideaFilter, borough: null });
            else setIdeaFilter({ ...ideaFilter, borough: "bronx" });
          }}
        />
      </svg>
    </div>
